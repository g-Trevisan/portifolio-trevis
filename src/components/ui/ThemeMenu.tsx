...
export function ThemeMenu({ theme, open, onToggle, onSelect }: ThemeMenuProps) {
  ...
  return (
    <div className="relative">
      <button className={cn(iconButton, 'gap-1')} type="button" onClick={onToggle} aria-label="Escolher tema" aria-expanded={open}>
      ...
      {open && (
        <div
          data-glass="menu"
          className="absolute right-0 top-[calc(100%+6px)] z-[70] flex min-w-40 flex-col gap-0.5 rounded-[10px] border border-[var(--border2)] bg-[var(--surface)] p-1.5 shadow-[0_12px_32px_rgba(0,0,0,.4)]"
        >
          {THEME_OPTIONS.map((option) => (
            ...
            >
              ...
              {theme === option.id && <Icon name="check" size={14} className="text-[var(--accent)]" />}
            </button>
          ))}
        </div>
      )}
